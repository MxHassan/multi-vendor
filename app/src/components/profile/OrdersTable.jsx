...
// import { ordersPageColumns } from '../../static/data.jsx'

const OrdersTable = ({ rows }) => {
  const columns = ['Order ID', 'Status', 'Items Qty', 'Total', 'Link']
  return (
    <Card className='h-full w-full   '>
      <CardBody className='overflow-x-scroll px-0 bg-light-background-secondary dark:bg-dark-background-main rounded-md duration-200'>
                ...
                <tr key={id}>
                  ...
                  <td className={classes}>
                    <div className='w-max'>
                      <Chip
                        ...
                        variant='ghost'
                        value={status}
                        color={status === 'Success' ? 'green' : status === 'Processing' ? 'orange' : 'red'}
                      />
                    </div>
                  </td>
                  <td className={classes}>
                  ...
                  <td className={classes}>
                    <Link to={`/user/orders/${id}`}>
                      <IconButton variant='text' className='text-light-text-primary dark:text-dark-text-primary'>
